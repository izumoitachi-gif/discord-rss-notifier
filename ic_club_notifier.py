...
毎日ニュース5分類_取得クエリ設計_厳重版.md のクエリバンクから、実データ
検証済みのGoogle Newsクエリを選抜して実装（+③④はNHK政治カテゴリ直RSS）。
2026-07-05、旧4ソース構成(direct_rss全般/gdelt/openalex)が実データ検証で
軒並み0件通過だったため全面差し替え。同日、①②が通常巡回で薄い（0〜1件）
と判明したため①に3本・②に2本クエリを追加増強（①8本・②7本・③④⑤5本）。
...
NEWS_TOPICS = {
    "①後期高齢者のAI挑戦": {
        ...
        "sources": [
            ...
            {"type": "google_news",
             "query": '("高齢者" OR シニア) (マイナポータル OR マイナンバー OR オンライン手続き) (支援 OR 相談 OR 使い方)'},
            {"type": "google_news",
             "query": '("高齢者" OR シニア) (キャッシュレス OR QR決済 OR 電子マネー) (使い方 OR 支援 OR トラブル OR 詐欺)'},
            {"type": "google_news",
             "query": '("高齢者" OR シニア) (音声入力 OR 読み上げ OR 音声アシスタント) (スマホ OR AI OR 使い方 OR 便利)'},
        ],
    },
    "②高齢者の老後不安を解消する完全ガイド": {
        ...
        "sources": [
            ...
            {"type": "google_news",
             "query": '(年金生活 OR 老後 OR 高齢者) (年金 OR 厚生年金 OR 国民年金 OR 支給額 OR 改正) (生活 OR 不安 OR 対策)'},
            {"type": "google_news",
             "query": '("高齢者" OR 老後) (住まい OR 賃貸 OR 空き家 OR 移住 OR 生活支援 OR 買い物弱者 OR 免許返納) (対策 OR 制度 OR 自治体 OR 相談)'},
        ],
    },
    "③和の国の羅針盤：高市政権の挑戦": {
